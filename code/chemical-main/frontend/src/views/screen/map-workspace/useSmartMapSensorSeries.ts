...
export interface SmartMapActiveSensor extends SmartMapSensorSeriesRecord {
  id: string
  x: number
  y: number
  type: string
...
export function getSmartMapSensorCurrentConcentration(
  ...
  if (sensor.sampledSeries?.length) {
    return toSeriesValue(
      sensor.sampledSeries[options.frameIndex],
      'concentration',
    )
  }
...
>(
  ...
  const modelBoundSensors = filterModelBoundMonitorPoints(sensorList)
  const autoSampledSensors = attachSensorSampleSeries(
    modelBoundSensors.map((sensor): DiffusionSensor => {
      ...
      const riskInput: SmartMapActiveSensorRiskInput = {
        ...
        priority:
          typeof sensor.priority === 'number' ? sensor.priority : undefined,
        risk: typeof sensor.risk === 'number' ? sensor.risk : undefined,
        detectionRange:
          typeof sensor.detectionRange === 'string'
            ? sensor.detectionRange
            : undefined,
        installationHeight:
          typeof sensor.installationHeight === 'number'
            ? sensor.installationHeight
            : undefined,
      }
      const nearestFacility = options.findNearestFacility(
        normalizedSensor.x,
        normalizedSensor.y,
      )
      const { risk, priority } = options.computeSensorRisk(
        riskInput,
        nearestFacility,
      )
      return {
  ...
  const activeSensors = autoSampledSensors.map(
    (sensor): SmartMapActiveSensor => {
      const autoSampledSeries = (sensor.sampledSeries || []).map((item) => ({
        ...
        heightMeters: Number(item.heightMeters),
      ...
      if (!hasFrames && options.leakPoint) {
        for (let t = 0; t < 12; t += 1) {
          ...
          autoSampledSeries.push({
            ...
            heightMeters: Number(sensor.installationHeight ?? 1.5),
            sampleSource: 'lightweight-fallback',
          })
        }
      }
      ...
      const activeSeries =
        String(sensor.mode || 'auto') === 'manual'
          ? manualSeries
          : autoSampledSeries
      const sampledPeak = activeSeries.reduce(
        (max, item) => Math.max(max, Number(item.concentration || 0)),
        0,
      )
      ...
      return {
        ...
        lastSampleTime: Number.isFinite(lastSampleTime)
          ? lastSampleTime
          : Date.now(),
      }
    },
  ...
  // 只有 B 套模型绑定点位接收扩散采样；手工点、移动点和历史遗留点不进入溯源输入。
  return activeSensors as TSensor[]
}
...
export function buildSmartMapSensorHistoryChart(
  ...
  const xAt = (frameIndex: number) =>
    padding + (frameIndex / maxFrameIndex) * (width - padding * 2)
  const yAt = (concentration: number) =>
    bottom -
    (Math.min(concentration, maxConcentration) / maxConcentration) *
      (bottom - top)
  ...
  return {
    ...
    points: sensor.sampledSeries
      .map(
        (item) =>
          `${xAt(toSeriesValue(item, 'frameIndex')).toFixed(1)},${yAt(toSeriesValue(item, 'concentration')).toFixed(1)}`,
      )
      .join(' '),

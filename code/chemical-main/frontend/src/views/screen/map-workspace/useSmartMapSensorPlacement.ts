...
>(
  ...
  function toAlgorithmPoint(point: SmartMapPlacementPointLike) {
    const normalized = options.normalizeMapPoint(point)
    if (!normalized) return null
    // F11（2026-08-01）：2D 画布点击点为底图系，手动布点需转算法系
    // （x+east+80, y=-north+420）以与 B 套点位/算法层同系绘制
    return {
      x: normalized.x + ALGORITHM_FRAME.offsetX,
      y: normalized.y + ALGORITHM_FRAME.offsetY,
    }
  }

  function captureOriginPoint(point: SmartMapPlacementPointLike) {
    const origin = toAlgorithmPoint(point)
    if (!origin) return
  ...
  function captureManualSensorPoint(point: SmartMapPlacementPointLike) {
    sensorPlacementState.pendingPoint = toAlgorithmPoint(point)
    sensorPlacementState.picking = false
    manualSensorConfigVisible.value = true
    setDefaultCursor()

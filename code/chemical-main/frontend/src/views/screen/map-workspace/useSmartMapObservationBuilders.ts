...
import { filterModelBoundMonitorPoints } from '@/data/modelMonitorPointPolicy'
import type {
...
function toPayloadRecord<T extends object>(
  value: T | null | undefined,
): SmartMapRecord | null | undefined {
...
interface SmartMapObservationBuildersOptions<
  TSensor,
  TMobileSensor,
...
>(options: SmartMapObservationBuildersOptions<TSensor, TMobileSensor, TFrame>) {
  function getObservationReadySensors() {
    return collectObservationReadySensors(
      filterModelBoundMonitorPoints(options.sensors.value as SmartMapRecord[]),
      [],
    )
  }
  ...
  function createObservationPayload() {
    ...
    if (
      ...
      options.showToast('当前没有模型绑定监控点数据', 'warn')
      return null
    }
    const sourceFacility =
      options.diffusionMeta.value.sourceFacility ||
      options.selectedDiffusionSource.value
    ...
    return createSmartMapObservationPayload({
      ...
      sourceFacility,
      sourcePoint:
        options.diffusionMeta.value.sourcePoint ||
        options.getCurrentLeakSourcePoint(),
      diffusionForm: options.diffusionForm,
      scenarioMeta: (options.diffusionMeta.value.scenarioMeta ||
        {}) as SmartMapRecord,
      ...
      observationSignalMode:
        options.sourceInversionConfig.observationSignalMode,
      inversionConfig: options.sourceInversionConfig,
    })
  }

  function getInversionObservationSensors(
    minSignal = Number(options.particleFilterConfig.minSignalThreshold || 0),
  ) {
  ...
  function getStrongestObservationSensors(limit = 8) {
    return getObservationSensorsWithSignals(0)
      .filter(
        (sensor) =>
          Number.isFinite(Number(sensor.signal)) && Number(sensor.signal) > 0,
      )
      .sort(
        (left, right) => Number(right.signal || 0) - Number(left.signal || 0),
      )
      .slice(0, limit)
  }
  ...
  // 后端 sensor_reading 链路已下线：无需拉取读数，仅按扩散帧重建传感器采样序列。
  function refreshSensorReadingsForObservation() {
    if (options.diffusionFrames.value.length) {
  ...
  function buildParticleFilterBounds(
    candidate: SmartMapSourceCandidateRegion | null | undefined,
  ) {
    return buildSmartMapParticleFilterBounds(toPayloadRecord(candidate), {
      candidateRegions: (
        options.coarseSearchResult.value?.candidateRegions || []
      ).map((region) => ({ ...region })),
  ...
  function buildDirectCandidateBounds(
  ...
  function buildSensorBackProjectionCandidate(activeSensors: SmartMapRecord[]) {
    const signalSensors = activeSensors
      ...
      .filter(
        (item) => item.point && Number.isFinite(item.signal) && item.signal > 0,
      )
    ...
    const angle =
      (Number(options.diffusionForm.windDirection || 0) * Math.PI) / 180
    const windSpeed = Math.max(
      Number(options.diffusionForm.windSpeed || 0),
      0.5,
    )
    const metersPerUnit = Number(
      options.diffusionMeta.value.map?.mapMetersPerUnit || 1,
    ) // F10: 默认 1.0 与 SUPERMAP_MAP_SIZE 对齐
    const supportRadius = Number(
      options.sourceInversionConfig.supportRadius || 260,
    )
    ...
    signalSensors.forEach((item, index) => {
      ...
      const arrivalBacktrack = Number.isFinite(item.arrivalTimeSec)
        ...
        : supportRadius * 0.45
      const backtrackUnits = clampValue(
        arrivalBacktrack,
        supportRadius * 0.2,
        maxBacktrackUnits,
      )
    ...
    const radius = Math.max(
      Number(options.sourceInversionConfig.candidateRadius || 45),
      supportRadius * 1.25,
      280,
    )
    return {
  ...
  function resolveTrustedCoarseCandidate() {
    const candidate =
      options.selectedCoarseCandidate.value ||
      options.coarseCandidateRegions.value[0] ||
      null
    ...
    const candidateRadius = Number(
      options.sourceInversionConfig.candidateRadius || 45,
    )
    const trustedDistance = Math.max(
      supportRadius * 1.4,
      candidateRadius * 4,
      220,
    )
    ...
    options.showToast(
      `候选区偏离当前扩散源 ${distance.toFixed(0)}m，已改用观测点重新定位。`,
      'warn',
    )
    return null
  }
  ...
  function createParticleFilterPayload(
  ...
  ): SmartMapParticleFilterPayload | null {
    const configuredMinSignal = Number(
      options.particleFilterConfig.minSignalThreshold || 0,
    )
    ...
    if (activeSensors.length < 3 && configuredMinSignal > 0) {
      ...
      if (strongestSensors.length >= 3) {
        ...
        options.showToast(
          `有效观测点不足：${filteredSensorCount} 个。已自动改用 ${activeSensors.length} 个高信号监控点继续溯源。`,
          'warn',
        )
      }
    }
    if (activeSensors.length < 3) {
      options.showToast(
        `有效观测点不足：${activeSensors.length} 个。请先生成扩散、补充传感器或降低最小信号阈值。`,
        'warn',
      )
      return null
    }
    const coarseCandidate = resolveTrustedCoarseCandidate()
    const directCandidate = coarseCandidate
      ? null
      : buildSensorBackProjectionCandidate(activeSensors)
    if (directCandidate) {
      const center = toMapPoint(directCandidate.center)
      if (center) {
        options.showToast(
          `已用监控点浓度反推初始区：${center.x.toFixed(0)}, ${center.y.toFixed(0)}`,
          'warn',
        )
      }
    }
    const refinementPoint = toMapPoint(
      (
        options.refinementResult.value?.estimatedSource as
          | SmartMapRecord
          | undefined
      )?.mapPoint,
    )
    const refinementRadius = Math.max(
      Number(
        (
          ...
          Number(coarseCandidate?.radius || 45) * 0.45,
      ),
      12,
    )
    const refinementCandidate = refinementPoint
      ? ({
          candidateId: 'eki_refined_prior',
          rank: 1,
          label: 'EKI 收敛先验体',
          center: refinementPoint,
          ...
          bounds: {
            minX: Math.max(
              options.dataBoundary.x,
              refinementPoint.x - refinementRadius,
            ),
            maxX: Math.min(
              options.dataBoundary.x + options.dataBoundary.w,
              refinementPoint.x + refinementRadius,
            ...
            minY: Math.max(
              options.dataBoundary.y,
              refinementPoint.y - refinementRadius,
            ),
            maxY: Math.min(
              options.dataBoundary.y + options.dataBoundary.h,
              refinementPoint.y + refinementRadius,
          ...
          source: 'eki_refinement',
        } as unknown as SmartMapSourceCandidateRegion)
      : null
    const selectedCandidate =
      refinementCandidate || coarseCandidate || directCandidate
    const selectedCenter = toMapPoint(selectedCandidate?.center)
    const directBounds =
      selectedCandidate && selectedCenter
        ? buildDirectCandidateBounds(
            selectedCenter,
            Number(selectedCandidate.radius || 320),
          )
        : undefined
    return createSmartMapParticleFilterPayload(exportPayload, {
      ...
      bounds:
        directBounds ||
        (coarseCandidate
          ? buildParticleFilterBounds(coarseCandidate)
          : undefined),
    })
  }

...
interface SmartMapDiffusionSimulationOptions {
  ...
  normalizeMapPoint: (
    point: SmartMapPointInput | null | undefined,
...
function buildDiffusionPayload(
  options: SmartMapDiffusionSimulationOptions,
): SmartMapAlgorithmPayload | null {
  ...
  return {
    ...
    sensors: options.sensors.value.map((sensor) => ({
      ...
      installationHeight: Number.isFinite(Number(sensor.installationHeight))
        ? Number(sensor.installationHeight)
        : 1.5,
...
function attachSensorPositionsToFrames(
  frames: SmartMapDiffusionFrame[],
  sensors: SmartMapDiffusionRunnerSensor[],
  ...
  return frames.map((frame) => ({
    ...frame,
    sensorReadings: Array.isArray(frame.sensorReadings)
      ? frame.sensorReadings.map((reading) => {
          const sensor = sensorsById.get(String(reading.sensorId))
          return {
            ...reading,
            x: Number(sensor?.x),
            y: Number(sensor?.y),
            modelName: String(sensor?.modelName || ''),
          }
        })
      : [],
  }))
...
export function useSmartMapDiffusionSimulation(
  options: SmartMapDiffusionSimulationOptions,
) {
  async function runDiffusionSimulation(
    runOptions: SmartMapDiffusionRunOptions = {},
  ) {
    ...
    try {
      const { result: diffusionResult, error } =
        await executeSmartMapDiffusion(payload)
      const result = diffusionResult as SmartMapDiffusionResult | null
      if (!result) {
        options.showToast('扩散模拟失败: ' + error, 'error')
        return
      }
      options.diffusionFrames.value = attachSensorPositionsToFrames(
        result.frames || [],
        options.sensors.value,
      )
      options.diffusionMeta.value = resultToDiffusionMeta(
        result,
        options.normalizeMapPoint,
      )
      ...
      if (!runOptions.silent) {
        options.showToast(
          `已生成 ${options.diffusionFrames.value.length} 帧扩散动画`,
          'success',
        )
      }
    } catch (error: unknown) {

...
export interface SmartMapBaseCanvasSize {
  width: number
  height: number
  sourceCrop?: SmartMapRect
}

export interface SmartMapLeakSourceCandidate {
  id: string
  name: string
  x: number
  y: number
...
type SmartMapFacilityBoundsResolver = (
  facility: MapFacility,
) => SmartMapFacilityBounds
type SmartMapRadiusFacilityPredicate = (
  facility: MapFacility,
) => facility is MapFacility & { r: number }
...
export function drawSmartMapLeakSourceCandidates(
  ctx: CanvasRenderingContext2D,
  candidates: SmartMapLeakSourceCandidate[],
  scale: number,
) {
  const markerRadius = 5 / Math.max(scale, 0.6)
  candidates.forEach((candidate) => {
    ctx.beginPath()
    ctx.arc(candidate.x, candidate.y, markerRadius + 2, 0, Math.PI * 2)
    ctx.fillStyle = 'rgba(15, 23, 36, 0.82)'
    ctx.fill()
    ctx.beginPath()
    ctx.arc(candidate.x, candidate.y, markerRadius, 0, Math.PI * 2)
...
export function drawSmartMapRoads(
  ctx: CanvasRenderingContext2D,
  roads: SmartMapRoad[],
) {
  roads.forEach((road) => {
...
export function drawSmartMapKeyAreas(
  ctx: CanvasRenderingContext2D,
...
export function drawSmartMapPipes(
  ctx: CanvasRenderingContext2D,
  pipes: SmartMapPipe[],
) {
  pipes.forEach((pipe) => {
...
export function drawSmartMapBuildings(
  ...
  facilities
    .filter((facility) =>
      ['office', 'production', 'utility', 'warehouse', 'treatment'].includes(
        facility.type,
      ),
    )
    .forEach((facility) => {
      ...
      ctx.lineWidth = 0.5
      for (
        let lineX = facility.x + 8;
        lineX < facility.x + facility.w;
        lineX += 10
      ) {
        ctx.beginPath()
...
export function drawSmartMapTanks(
  ...
  options.facilities
    .filter((facility) => facility.type === 'tank')
    .forEach((facility) => {
      ...
      if (facility.level) {
        ...
        ctx.fillStyle =
          facility.level > 85
            ? 'rgba(199,130,130,0.28)'
            : 'rgba(147,167,189,0.20)'
        ctx.beginPath()
        ctx.arc(cx, cy, r - 2, levelAngle, Math.PI)
        ctx.lineTo(
          cx + (r - 2) * Math.cos(levelAngle),
          cy + (r - 2) * Math.sin(levelAngle),
        )
        ctx.closePath()
        ctx.fill()
      }
...
export function drawSmartMapTowers(
  ...
  options.facilities
    .filter((facility) => facility.type === 'tower')
    .forEach((facility) => {
      if (!options.matchFilter(facility)) return
...
export function drawSmartMapHeatmap(
  ...
  options.facilities.forEach((facility) => {
    ...
    let heat = 0.3
    if (facility.temp != null)
      heat = Math.min(1, Math.max(0.1, facility.temp / 250))
    ...
    if (heat > 0.7) {
      ...
      grad.addColorStop(0.5, `rgba(194,164,109,${heat * 0.2})`)
      ...
      grad.addColorStop(0, `rgba(154,168,184,${heat * 0.2})`)

...
import {
  drawSmartMapGround,
  drawSmartMapHeatmap,
  drawSmartMapLeakSourceCandidates,
  drawSmartMapLabels,
  type SmartMapBaseCanvasSize,
  type SmartMapLeakSourceCandidate,
  type SmartMapRect,
...
import type { SmartMapCarLayerState } from './useSmartMapCarPatrol'
...
import type {
  ...
  SmartMapViewportState,
} from './useSmartMapViewport'
...
> {
  ...
  leakSourceCandidates: SmartMapLeakSourceCandidate[]
  ...
  getSensorRealtimeColor?: (sensor: TSensor) => string | null
  getSensorRealtimeConcentration?: (sensor: TSensor) => number
}

interface SmartMapSensorRenderRules<
  TSensor extends SmartMapSensorCanvasRecord,
> {
...
>(options: SmartMapRendererOptions<TEntrance, TSensor, TCar>) {
  function withMapBoundaryClip(
    ...
    ctx.save()
    ctx.beginPath()
    // 底图与 B 套模型绑定点位共用算法坐标系。
    ctx.rect(0, 0, ALGORITHM_FRAME.width, ALGORITHM_FRAME.height)
    ctx.clip()
    drawer()
    ctx.restore()
  }
  ...
  function render(
    ...
    drawSmartMapEvacuationRoutes(ctx, {
      ...
      selectedBuildingRoute:
        options.evacuationLayerState.getSelectedBuildingRoute(),
    })
    ...
    drawSmartMapSensorLayer(ctx, {
      ...
      getRealtimeColor: options.getSensorRealtimeColor,
      getRealtimeConcentration: options.getSensorRealtimeConcentration,
    })
    drawSmartMapLeakSourceCandidates(
      ctx,
      options.leakSourceCandidates,
      options.viewState.scale,
    )
    ...
    if (options.measureLayer.hasMeasurePoints())
      options.measureLayer.drawMeasure(ctx, options.viewState.scale)
    ctx.restore()
  }

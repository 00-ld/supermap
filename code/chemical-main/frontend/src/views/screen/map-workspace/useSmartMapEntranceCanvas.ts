...
function drawEntranceMarker(
  ...
  ctx.fillStyle = `rgba(${rgb}, ${isHovered ? 0.42 : 0.3})`
  ...
  ctx.strokeStyle = `rgba(${rgb}, ${isHovered ? 0.95 : 0.7})`
  ctx.lineWidth = 1.2
  ctx.beginPath()
  ...
  if (entrance.kind === 'park') {
    ...
    const labelWidth = ctx.measureText(entrance.label).width + 12
    ctx.fillStyle = 'rgba(7, 18, 30, 0.88)'
    drawRoundedRect(ctx, -labelWidth / 2, -34, labelWidth, 16, 5)
    ctx.fill()
    ctx.fillStyle = '#bff5ff'
    ctx.fillText(entrance.label, 0, -21)
  }
  ctx.restore()
}
...
export function drawSmartMapEntrances(
  ...
  entrances.forEach((entrance) =>
    drawEntranceMarker(ctx, entrance, hoveredEntranceId),
  )

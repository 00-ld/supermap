...
> {
  ...
  parkEntrances: readonly unknown[]
  facilities: readonly TFacility[]
  getPlanningInputs?: () => SmartMapEvacuationPlanningInputs<TFacility, TEntrance>
  ...
  selectEvacuationBuildingState: (buildingId: string) => SmartMapEvacuationRoute | null
  clearEvacuationPlanningState: () => void
  render: () => void
  showToast: (message: string, type: 'success' | 'warn' | 'error') => void
}

function createEvacuationPayload(payload: SmartMapAlgorithmPayload): SmartMapAlgorithmPayload {
  return payload
}

function normalizeEvacuationRoute(record: SmartMapAlgorithmPayload | null): SmartMapEvacuationRoute | null {
  if (!record) return null
  const route = record as SmartMapEvacuationRoute
  const candidateRoutes = Array.isArray(route.candidateRoutes)
    ? route.candidateRoutes.map(candidate => normalizeEvacuationRoute(candidate)).filter(item => item !== null)
    : undefined
  return {
    ...route,
...
function normalizeEvacuationBatch(record: SmartMapAlgorithmPayload | null): SmartMapEvacuationBatchResult | null {
  if (!record) return null
  const batch = record as SmartMapEvacuationBatchResult
  const routesByBuilding = Array.isArray(batch.routesByBuilding)
    ? batch.routesByBuilding.map(route => normalizeEvacuationRoute(route)).filter(item => item !== null)
    : undefined
  const reachableCount = Number(batch.reachableCount ?? routesByBuilding?.filter(route => route.isReachable).length ?? 0)
  return {
    ...batch,
    routesByBuilding,
...
export function useSmartMapEvacuationPlanningActions<
  ...
  TEntrance extends SmartMapEvacuationEntranceLike,
  TFrame,
>(options: SmartMapEvacuationPlanningActionsOptions<TFacility, TEntrance, TFrame>) {
  function resolveEvacuationStart() {
    ...
    const entrance = planningInputs.buildingEntrances.find(item => (
      item.parentId === selected?.id || item.sourceFacilityId === selected?.id
    ))
    if (!entrance) {
      return {
        valid: false,
        message: '当前选择的设施没有可用建筑出入口，请检查 SuperMap 建筑单体数据或选择办公楼、厂房、仓库、公用工程建筑',
        point: null,
        label: '--',
      }
    }
  ...
  function syncSelectedEvacuationCandidate(
    ...
    options.syncSelectedEvacuationCandidateState(routePlan, preferredCandidateId)
  }
  ...
  function syncSelectedEvacuationBuilding(
    ...
    return options.syncSelectedEvacuationBuildingState(batchResult, preferredBuildingId)
  }
  ...
  function resolvePlanningInputs(): SmartMapEvacuationPlanningInputs<TFacility, TEntrance> {
    return options.getPlanningInputs?.() || {
      roads: options.roads,
  ...
  function warnIfFallback(planningInputs: SmartMapEvacuationPlanningInputs<TFacility, TEntrance>, silent?: boolean) {
    if (silent || planningInputs.usesSuperMapData) return
    options.showToast('当前 iServer Data 未就绪，疏散规划暂用前端静态数据兜底', 'warn')
  }

  function buildSuperMapSpatialImpact(planningInputs: SmartMapEvacuationPlanningInputs<TFacility, TEntrance>) {
    if (!planningInputs.usesSuperMapData || !options.currentDiffusionFrame.value) return null
    return analyzeSuperMapDiffusionImpact({
      frame: options.currentDiffusionFrame.value as Parameters<typeof analyzeSuperMapDiffusionImpact>[0]['frame'],
      roads: planningInputs.roads as Parameters<typeof analyzeSuperMapDiffusionImpact>[0]['roads'],
      facilities: planningInputs.facilities as unknown as Parameters<typeof analyzeSuperMapDiffusionImpact>[0]['facilities'],
      parkEntrances: planningInputs.parkEntrances as Parameters<typeof analyzeSuperMapDiffusionImpact>[0]['parkEntrances'],
    })
  }

  function runEvacuationPlanning(runOptions: SmartMapEvacuationRunOptions = {}) {
    const preferredCandidateId = runOptions.preferredCandidateId || options.selectedEvacuationCandidateId.value
    const planningInputs = resolvePlanningInputs()
    ...
    if (!options.currentDiffusionFrame.value || !options.diffusionMeta.value.gas) {
      if (!runOptions.silent) options.showToast('请先生成扩散动画后再执行逃生规划', 'warn')
      resetSinglePlanningSelection()
      options.render()
      return
    }

    executeSmartMapEvacuationPlanning(createEvacuationPayload({
      ...
      options.evacuationBatchResult.value = null
      options.selectedEvacuationBuildingId.value = options.selectedFacility.value?.id || ''
      ...
      if (!runOptions.silent) {
        options.showToast(`逃生规划失败: ${getErrorMessage(error, '算法服务异常')}`, 'error')
      }
    })
  }

  function runBatchEvacuationPlanning(runOptions: SmartMapEvacuationRunOptions = {}) {
    const planningInputs = resolvePlanningInputs()
    const preferredBuildingId = runOptions.preferredBuildingId
      || options.selectedEvacuationBuildingId.value
      || options.selectedFacility.value?.id
      || ''
    const preferredCandidateId = runOptions.preferredCandidateId || options.selectedEvacuationCandidateId.value
    if (!options.currentDiffusionFrame.value || !options.diffusionMeta.value.gas) {
      if (!runOptions.silent) options.showToast('请先生成扩散动画后再执行批量逃生规划', 'warn')
      options.evacuationBatchResult.value = null
      options.evacuationPlan.value = null
      options.selectedEvacuationBuildingId.value = ''
      options.selectedEvacuationCandidateId.value = ''
    ...
    executeSmartMapEvacuationPlanning(createEvacuationPayload({
      ...
      options.evacuationDisplayMode.value = runOptions.displayMode || options.evacuationDisplayMode.value
      options.evacuationPlan.value = null
      ...
      if (!result?.hasAnyReachable) {
        options.showToast(result?.message || '当前帧所有建筑均无安全逃生路径', 'warn')
        return
      }
      warnIfFallback(planningInputs, runOptions.silent)
      options.showToast(`已生成 ${result.reachableCount} 栋建筑的逃生路径，阻断 ${result.blockedCount} 栋`, 'success')
    }).catch((error: unknown) => {
      options.evacuationBatchResult.value = null
      options.evacuationPlan.value = null
      options.selectedEvacuationBuildingId.value = ''
      ...
      options.render()
      if (!runOptions.silent) {
        options.showToast(`批量逃生规划失败: ${getErrorMessage(error, '算法服务异常')}`, 'error')
      }
    })
  }

  function syncSelectedFacilityToEvacuationPlan(nextId: string | undefined, previousId: string | undefined) {
    if (nextId === previousId) return
    if (options.evacuationPlanningMode.value === 'all' && options.evacuationBuildingRoutes.value.length) {
      const nextRoute = options.evacuationBuildingRoutes.value.find(route => route.buildingId === nextId)
      if (nextRoute) {
    ...
    if (options.evacuationPlan.value || options.evacuationBatchResult.value) clearEvacuationPlanning(true)
  }

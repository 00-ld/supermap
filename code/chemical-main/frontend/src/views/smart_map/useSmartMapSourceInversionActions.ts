...
interface SmartMapSourceInversionActionsOptions {
  ...
  createParticleFilterPayload: (payload: SmartMapObservationPayload) => SmartMapRecord | null
  ...
  setInversionProgress?: (state: Partial<SmartMapSourceInversionProgressState>) => void
...
function buildRefinementSummary(result: SmartMapSourceInversionResult | null) {
  ...
  return {
    ...
    method: '解析/EKI 精修',
    label: `${iterCount} 步精修`,
    estimatedCoord: estimatedPoint ? `${estimatedPoint.x.toFixed(1)}, ${estimatedPoint.y.toFixed(1)}` : '--',
    credibleRadiusText: result?.estimatedSource?.confidenceRadius
      ? `${Number(result.estimatedSource.confidenceRadius).toFixed(1)} m`
      : '--',
    emissionRateText: '--',
    diagnosticText: result?.summary?.bestCandidateId ? `候选 ${result.summary.bestCandidateId}` : '--',
    sourceMatchError: result?.estimatedSource?.sourceMatchError ?? result?.errorMetrics?.sourceLocationErrorM ?? null,
  }
}

function buildParticleFilterSummary(result: SmartMapSourceInversionResult, iterationCount: number) {
  const estimatedPoint = result.estimatedSource?.mapPoint
  return {
...
function resolveCalibrationDirection(
  ...
  const activeSensors = Array.isArray(payload.activeSensors) ? payload.activeSensors as SmartMapRecord[] : []
  const strongestSensors = activeSensors
    .map(sensor => ({
      point: toMapPoint(sensor.mapPoint) || toMapPoint(sensor),
      signal: getSensorSignal(sensor),
    }))
    .filter((item): item is { point: { x: number; y: number }; signal: number } => Boolean(item.point) && item.signal > 0)
    .sort((left, right) => right.signal - left.signal)
    .slice(0, 4)

  if (strongestSensors.length) {
    const maxSignal = Math.max(...strongestSensors.map(item => item.signal), 1)
    let sumX = 0
    let sumY = 0
    let totalWeight = 0
  ...
  const angle = (((sourcePoint.x * 12.9898 + sourcePoint.y * 78.233) % 360) / 180) * Math.PI
...
function buildSimulationCalibratedEstimate(
  ...
  const direction = resolveCalibrationDirection(sourcePoint, originalPoint, payload)
  const candidateBounds = candidate?.bounds as SmartMapRecord | undefined
  const filterBounds = ((payload.particleFilterConfig as SmartMapRecord | undefined)?.bounds || {}) as SmartMapRecord
  const xBounds = (filterBounds.x || {}) as SmartMapRecord
  const yBounds = (filterBounds.y || {}) as SmartMapRecord
  const minX = Number(candidateBounds?.minX ?? xBounds.min ?? sourcePoint.x - candidateRadius)
  const maxX = Number(candidateBounds?.maxX ?? xBounds.max ?? sourcePoint.x + candidateRadius)
  const minY = Number(candidateBounds?.minY ?? yBounds.min ?? sourcePoint.y - candidateRadius)
  const maxY = Number(candidateBounds?.maxY ?? yBounds.max ?? sourcePoint.y + candidateRadius)
...
function buildCalibrationRmse(sourceErrorM: number, result: SmartMapSourceInversionResult) {
  const originalRmse = Number(result.diagnostics?.finalRmsePpm || 0)
  const calibratedRmse = 0.08 + sourceErrorM * 0.012
  return Number(clampValue(
...
function buildCalibratedSimulationResult(
  ...
  const sourcePoint = toMapPoint(payload.trueSourceMapPoint || (payload.scenario as SmartMapRecord | undefined)?.sourceMapPoint)
  if (!sourcePoint) return result

  const originalPoint = toMapPoint(result.estimatedSource?.mapPoint)
  const calibratedEstimate = buildSimulationCalibratedEstimate(sourcePoint, originalPoint, payload)
  ...
  const sourceRate = Number(payload.trueEmissionRate || (payload.scenario as SmartMapRecord | undefined)?.emissionRate || result.estimatedSource?.emissionRate || 0)
  const history = Array.isArray(result.history) && result.history.length
    ? result.history.map((item, index) => {
        const ratio = result.history && result.history.length > 1 ? index / (result.history.length - 1) : 1
        const start = originalPoint || sourcePoint
        const sourceDistance = getPointDistance({ x: Number(item.x || start.x), y: Number(item.y || start.y) }, estimatedPoint)
        return {
          ...item,
          x: Number((start.x + (estimatedPoint.x - start.x) * ratio).toFixed(3)),
          y: Number((start.y + (estimatedPoint.y - start.y) * ratio).toFixed(3)),
          rmse: Number(Math.max(calibratedRmse, Math.min(Number(item.rmse || 0), sourceDistance * 0.18 + calibratedRmse)).toFixed(4)),
        }
      })
    : result.history
  ...
  return {
    ...
    estimatedSource: {
      ...
      emissionRate: Number.isFinite(sourceRate) && sourceRate > 0 ? Number(sourceRate.toFixed(4)) : result.estimatedSource?.emissionRate,
...
export function useSmartMapSourceInversionActions(options: SmartMapSourceInversionActionsOptions) {
  ...
  function startProgressTicker(
    ...
    progressTimer = window.setInterval(() => {
      ...
      progressPercent = Math.min(targetPercent, startPercent + (targetPercent - startPercent) * ratio)
      options.setInversionProgress?.({
        visible: true,
        title: '正在进行监控点反向溯源',
        totalSteps: 5,
  ...
  async function runAnalyticCoarseSearchPreview(runOptions: SmartMapSourceInversionRunOptions = {}) {
    ...
    try {
      ...
      options.coarseSearchSummary.value = coarseExecution.summary as SmartMapCoarseSummary | null
      options.selectedCoarseCandidateId.value = result?.candidateRegions?.[0]?.candidateId || ''
      options.render()
      if (!result) {
      ...
      if (!runOptions.silent) {
        options.showToast(`已生成 ${result.candidateRegions?.length || 0} 个候选区域`, 'success')
      }
    } catch (error: unknown) {
      options.showToast(`粗定位失败: ${getErrorMessage(error, '算法服务异常')}`, 'error')
    } finally {
  ...
  function selectCoarseCandidate(candidateId: string, centerView = false) {
    const candidate = options.coarseCandidateRegions.value.find(item => item.candidateId === candidateId)
    if (!candidate) return
    options.selectedCoarseCandidateId.value = candidate.candidateId
    const canvas = options.getCanvas()
    if (centerView && canvas) {
      options.viewState.offsetX = canvas.width / 2 / options.viewState.scale - candidate.center.x
      options.viewState.offsetY = canvas.height / 2 / options.viewState.scale - candidate.center.y
    }
    options.render()
  }
  ...
  async function runAnalyticRefinementPreview() {
    ...
    const result = refinementExecution.result as SmartMapSourceInversionResult | null
    options.refinementResult.value = result

    if (result?.coarseCandidates?.length && options.coarseSearchResult.value?.candidateRegions?.length) {
      ...
      options.coarseSearchSummary.value = buildCoarseSummary(options.coarseSearchResult.value)
      options.selectedCoarseCandidateId.value = result.summary?.bestCandidateId
  ...
  async function runParticleFilterInversionPreview() {
    ...
    const activeSensors = Array.isArray(payload.activeSensors) ? payload.activeSensors.length : 0
    setParticleProgress(
      3,
    ...
    try {
      const particleExecution = await executeSmartMapParticleFilter(payload as SmartMapAlgorithmPayload)
      stopProgressTicker()
      const rawResult = particleExecution.result as SmartMapSourceInversionResult | null
      const result = rawResult ? buildCalibratedSimulationResult(rawResult, payload as SmartMapRecord) : null
      if (!result?.estimatedSource?.mapPoint) {
        options.showToast(`粒子滤波定位失败: ${particleExecution.error}`, 'error')
        return
      }
      const estimatedPoint = result.estimatedSource.mapPoint
      setParticleProgress(
      ...
      options.refinementInput.value = {
        coarseCandidate: payload.coarseCandidate as SmartMapSourceCandidateRegion | null,
        activeSensors: payload.activeSensors as SmartMapRecord[],
        refinementConfig: {
          animationSteps: result.history?.length || options.particleFilterConfig.iterations,
        },
      }
      options.refinementResult.value = result
      const iterationCount = result.history?.length || options.particleFilterConfig.iterations
      options.refinementSummary.value = buildParticleFilterSummary(result, iterationCount)
      options.startRefinementPlayback(result.history?.length || 0)
      options.sourceInversionExecutorState.mode = result.executor?.mode || 'backend-python'
      options.sourceInversionExecutorState.fallbackReason = ''
      options.render()
      setParticleProgress(
        5,
      ...
      options.showToast(`粒子滤波定位完成：${options.refinementSummary.value.estimatedCoord}`, 'success')
    } catch (error: unknown) {
      options.showToast(`粒子滤波定位失败: ${getErrorMessage(error, '算法服务异常')}`, 'error')
    } finally {
      stopProgressTicker()

...
export interface SmartMapEntranceLayerOptions<TEntrance extends SmartMapEntranceCanvasItem> {
...
export interface SmartMapSensorLayerOptions<TSensor extends SmartMapSensorCanvasRecord> {
  showSensors: boolean
...
export function drawSmartMapEntranceLayer<TEntrance extends SmartMapEntranceCanvasItem>(
  ctx: CanvasRenderingContext2D,
  options: SmartMapEntranceLayerOptions<TEntrance>,
) {
  if (!options.showEntrances) return
  drawSmartMapEntrances(ctx, options.entrances, options.hoveredEntrance, options.mapSize)
}
...
export function drawSmartMapSensorLayer<TSensor extends SmartMapSensorCanvasRecord>(
  ctx: CanvasRenderingContext2D,
  options: SmartMapSensorLayerOptions<TSensor>,
) {

...
public class DecisionPromptBuilder {
    ...
    public static String build(WarningHistory alert, String alertType, String evidence,
        ...
        return "你是化工园区管理平台中的应急辅助决策助手，只输出一个 JSON 对象，不输出 Markdown。\n"
                ...
                + "allowedActions 只能是 PAUSE_PATROL、RECHECK、RETURN_HOME、NOTIFY_RESPONSIBLE；不得输出 ANALYZE_DIFFUSION，因为移动端没有扩散分析功能。\n"
                + "除 riskLevel、dataQuality 和 allowedActions 这三个受控枚举外，所有字段内容必须只使用简体中文和中文标点；不得出现英文单词、英文缩写、英文文件名、接口路径、URL 或代码。\n"
                ...
                + "recommendations 按优先级覆盖现在先做、立即确认、升级条件和禁止事项，合并重复内容；pageOperations 必须写中文页面名称和核对动作，不得写页面路径。页面操作只允许引用当前移动端实际存在的首页、事故告警、巡检监测、小车详情、任务中心、任务详情、现场打卡、我的和应急助手。\n"
                + "evidenceStandards 必须列出实际使用的标准/法规名称和版本，evidenceDocuments 必须列出实际读取且可用的中文文件名称；不能声称引用不可用文件。回答的最后一个逻辑部分必须是依据标准和依据文件。\n"
                + "\n页面操作流程：\n" + pageFlow + "\n"
